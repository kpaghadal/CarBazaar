...
export function AdminUsers() {
  ...
  useEffect(() => {
    adminAPI.getUsers()
      .then((data) => {
        // Normalize: add .status field (not in DB model, but UI uses it)
        setUsers(data.map((u) => ({ ...u, id: u._id, status: 'Active', listings: 0 })));
      })
      .catch(console.error)
  ...
  const handleDelete = async () => {
    try {
      await adminAPI.deleteUser(modal.user._id || modal.user.id);
      setUsers(users.filter((u) => (u._id || u.id) !== (modal.user._id || modal.user.id)));
      setModal(null);
    } catch (err) {
      alert(err.message);
    }
  ...
  const handleAdd = (form) => {
    ...
    setUsers([{ ...form, id: newId, _id: newId, listings: 0 }, ...users]);
    setModal(null);
  };

  const handleEdit = (form) => {
    setUsers(users.map((u) =>
      (u._id || u.id) === (modal.user._id || modal.user.id) ? { ...u, ...form } : u
    ));
    setModal(null);
  };
